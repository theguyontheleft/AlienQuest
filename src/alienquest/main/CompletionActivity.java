 ...
 * 
 * 
 *          I (Jimmy) chose to store the highscores in a string stored as
 *          a preference, rather than a database because I wanted to practice with
...
public class CompletionActivity extends Activity
{
    ...
    protected void onCreate( Bundle savedInstanceState )
    {
        ...
        if ( b != null )
        {
            int scoreTotal = Integer.parseInt( b.get( getString( R.string.key_score ) ).toString() );
            boolean gameComplete =
                    Boolean.parseBoolean( b.get( getString( R.string.key_completed ) ).toString() );
            boolean callCompletedMethod =
                    Boolean.parseBoolean( b.get( getString( R.string.key_display_completed) ).toString() );
            ...
            if ( score > highscores_.keyAt( 4 ) )
            {
                ...
                Toast.makeText(
                        ...
                        "Congradulations " + userName_ + "! Your score of "
                                + score
                                + " is a top five number!", Toast.LENGTH_SHORT )
                        .show();
            }
        }
        else
    ...
    private void getTopFiveScores()
    {
        ...
        {
            highscores_.put( Integer.parseInt( arrayOfScores[j] ),
                    arrayOfNames[j] );
        }
